...
static NSManagedObjectContext *MagicalRecordBackgroundFetchContext;
    ...
    if (MagicalRecordDefaultContext == nil)
    {
        ...
        dispatch_sync(dispatch_get_main_queue(), ^{
            ...
            /*[defaultContext performBlockAndWait:^{
             ...
             MRLogVerbose(@"Created new context %@ with store coordinator: %@", [defaultContext MR_workingName], coordinator);
             }];*/
            //[self MR_setRootSavingContext:defaultContext];
        ...
        dispatch_sync(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),^{
            ...
            /*NSManagedObjectContext *backgroundFetchContext = [NSManagedObjectContext MR_newPrivateQueueContext];
            [backgroundFetchContext setParentContext:[NSManagedObjectContext MR_rootSavingContext]];*/

            NSManagedObjectContext *backgroundFetchContext = [self MR_contextWithStoreCoordinator:coordinator];
            
            [backgroundFetchContext setMergePolicy:NSMergeByPropertyObjectTrumpMergePolicy];

            MagicalRecordBackgroundFetchContext = backgroundFetchContext;
...
+ (NSManagedObjectContext *) MR_backgroundFetchContext;
{
    NSAssert(MagicalRecordBackgroundFetchContext != nil, @"Background Fetch saving context is nil! Did you forget to initialize the Core Data Stack?");
    return MagicalRecordBackgroundFetchContext;
}
...
+ (void)rootContextDidSave:(NSNotification *)notification
{
    ...
    if ([NSThread isMainThread] == NO)
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self rootContextDidSave:notification withObjectIDs: objectIDs];
        });
        
        [MagicalRecordBackgroundFetchContext performBlock:^{
            [self rootContextDidSave2:notification withObjectIDs: objectIDs];
        }];
    ...
    for (NSManagedObject *object in [[notification userInfo] objectForKey:NSUpdatedObjectsKey])
    {
        NSManagedObject* mo = [[self MR_defaultContext] existingObjectWithID:[object objectID] error:nil];
        if (mo != nil)
        {
...
#if DEBUG
    NSTimeInterval currentTimeEnd = [[NSDate date] timeIntervalSince1970] * 1000;

    NSLog(@"Saving root context in main thread took %@ ms", @(currentTimeEnd - currentTime));
#endif
}

+ (void)rootContextDidSave:(NSNotification *)notification withObjectIDs:(NSArray *)objectIDs
{
#if DEBUG
    NSTimeInterval currentTime = [[NSDate date] timeIntervalSince1970] * 1000;
#endif
    ...
    {
        NSManagedObject* mo = [[self MR_defaultContext] existingObjectWithID: objectID error:nil];
        if (mo != nil)
        {
            [mo willAccessValueForKey:nil];
        } else
        {
            NSLog(@"Nonexisting Object %@", objectID);
        }
    }
...
+ (void)rootContextDidSave2:(NSNotification *)notification withObjectIDs:(NSArray *)objectIDs
{
#if DEBUG
    NSTimeInterval currentTime = [[NSDate date] timeIntervalSince1970] * 1000;
#endif
//    for (NSManagedObjectID *objectID in objectIDs)
    {
        for (NSManagedObject *object in [[notification userInfo] objectForKey:NSUpdatedObjectsKey])
        {
            NSManagedObject* mo = [[self MR_backgroundFetchContext] existingObjectWithID:[object objectID] error:nil];
            if (mo != nil)
            {
                [mo willAccessValueForKey:nil];
            }else
            {
                NSLog(@"Nonexisting Object %@", [object objectID]);
            }
    ...
    @try
    {
        [[self MR_backgroundFetchContext] mergeChangesFromContextDidSaveNotification:notification];
    } @catch (NSException *exception)
    {
        NSLog(@"Merge Failed: %@", exception);
    }
    ...
    NSLog(@"Saving Background Fetch context took %@ ms", @(currentTimeEnd - currentTime));
#endif
}
...
-(void)performBlockAndWait:(void (^)(void))block  rethrowExceptions:(BOOL)rethrow
{
    __block NSException* innerExcpetion;
    ...
    [self performBlockAndWait:^{
        @try
        {
            ...
            innerExcpetion = exception;
        }
    }];
    if (innerExcpetion != nil && rethrow)
    {
        @throw innerExcpetion;
    }
