...
public protocol ImageSlideShowProtocol {
  ...
  func performShare(fromButton: UIBarButtonItem) -> Void
}
...
open class ImageSlideShowViewController: UIPageViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate {
  static var imageSlideShowStoryboard:UIStoryboard = UIStoryboard(name: "ImageSlideShow", bundle: Bundle(for: ImageSlideViewController.self))
  ...
  open var enableSharing: Bool = true
  ...
  override open func viewDidLoad() {
    ...
    navigationItem.leftBarButtonItem = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(dismiss(sender:)))
    navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .save, target: self, action: #selector(shareImage(sender:)))
  ...
  @objc open func shareImage(sender: AnyObject?) {
    if let slides = slides, slides.count >= _currentIndex,  let rbbi = navigationItem.rightBarButtonItem {
      let slide = slides[_currentIndex]
      slide.performShare(fromButton: rbbi)
    }
  }
