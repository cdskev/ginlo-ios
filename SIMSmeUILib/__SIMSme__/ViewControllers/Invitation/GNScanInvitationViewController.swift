//
//  GNScanInvitationViewController.swift
//  SIMSmeUILib
//
...
class GNScanInvitationViewController: DPAGViewControllerBackground {
    ...
    private var scanWorker: DPAGScanInvitationWorker?
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if self.scanWorker == nil {
            self.scanWorker = DPAGScanInvitationWorker(presentationView: self.viewScanner, scanDelegate: self)
...
extension GNScanInvitationViewController: DPAGScanInvitationDelegate {
...
private protocol DPAGScanInvitationDelegate: AnyObject {
    func scanFinishedWithSuccess(_ text: String)
    func scanFinishedWithFail()
}

private class DPAGScanInvitationWorker: NSObject {
    private var presentationView: UIView
    private weak var scanDelegate: DPAGScanInvitationDelegate?
    private var scanWorkerDelegate: DPAGScanInvitationWorkerDelegate?
    private var capture: ZXCapture?

    var isValidating = false

    init(presentationView: UIView, scanDelegate: DPAGScanInvitationDelegate) {
    ...
    func validateResult(_ text: String) {
        let isValid = DPAGApplicationFacade.contactsWorker.validateScanResult(text: text)
...
private class DPAGScanInvitationWorkerDelegate: NSObject, ZXCaptureDelegate {
    weak var scanWorker: DPAGScanInvitationWorker?

    init(scanWorker: DPAGScanInvitationWorker) {
