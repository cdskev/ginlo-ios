...
//    Thread.sleep(forTimeInterval: 20.0)
    ...
    for inputItem in extensionContext.inputItems {
      NSLog("ShareExtension::InputItem == \(inputItem)")
  ...
  private func dismissUnsupportedMedia() {
    self.hideProgressHUD(animated: true) { [weak self] in
      guard let strongSelf = self else { return }
      strongSelf.presentErrorAlert(alertConfig: UIViewController.AlertConfigError(messageIdentifier: DPAGLocalizedString("share.extension.medias.unsupportedFormatError"), okActionHandler: { [weak self] _ in
        guard let strongSelf = self else { return }
