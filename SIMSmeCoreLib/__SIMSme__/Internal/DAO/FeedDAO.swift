...
class FeedDAO: FeedDAOProtocol {
    ...
    func setFeedSubscribed(feedGuid: String) {
        DPAGApplicationFacade.persistance.saveWithBlock { localContext in
            ...
            guard let feed = SIMSChannel.findFirst(byGuid: feedGuid, in: localContext) else {
                return
            }
            ...
            guard (feed.subscribed?.boolValue ?? false) == false else {
                return
            }
    ...
    func setFeedUnsubscribed(feedGuid: String, feedType: DPAGChannelType) {
        DPAGApplicationFacade.persistance.saveWithBlock { localContext in
            ...
            if feedType == .service, let rootOptions = feed.rootOptions {
                for option in rootOptions {
                    (option as? SIMSChannelOption)?.reset()
                }
            }
    ...
    func assetsWithType(feedGuid: String, assetTypes: [DPAGChannel.AssetType]) -> [DPAGChannel.AssetType: Any] {
        ...
        DPAGApplicationFacade.persistance.loadWithBlock { localContext in
            ...
            guard let channelAssets = SIMSChannel.findFirst(byGuid: feedGuid, in: localContext)?.assets else {
                return
            }

            for asset in channelAssets {
                guard let assetTypeRaw = asset.type, let assetType = DPAGChannel.AssetType(rawValue: assetTypeRaw), let assetData = asset.data, assetTypes.contains(assetType) else {
                    continue
                }
    ...
    func getFeedOptions(forFeedGuids feedGuids: [String]) -> (feedGuidsMandantory: [String], feedGuidsRecommended: [String: String]) {
        ...
        DPAGApplicationFacade.persistance.loadWithBlock { localContext in
            ...
            for feedGuid in feedGuids {
                guard let channel = SIMSChannel.findFirst(byGuid: feedGuid, in: localContext) else {
                    continue
                }
    ...
    func saveServerFeeds(feedsWithChecksum: [[String: Any]], feedType: DPAGChannelType) -> (feedGuids: [String], feedGuidsToUpdate: [String]) {
        ...
        DPAGApplicationFacade.persistance.saveWithBlock { localContext in
            ...
            for feedWithChecksumDict in feedsWithChecksum {
                ...
                guard let channel = SIMSChannel.findFirst(byGuid: channelGuid, in: localContext) ?? SIMSChannel.mr_createEntity(in: localContext) else {
                    continue
                }
    ...
    private func markDeletedChannels(feedGuidsExisting: [String], feedType: DPAGChannelType, in localContext: NSManagedObjectContext) {
        let predicateFeedsForFeedType =
            NSComparisonPredicate(leftExpression: NSExpression(forKeyPath: \SIMSChannel.feedType), rightExpression: NSExpression(forConstantValue: feedType.rawValue))

        guard let feedsForFeedType = SIMSChannel.mr_findAll(with: predicateFeedsForFeedType, in: localContext) else {
            return
        }
        ...
        DPAGApplicationFacade.persistance.loadWithBlock { localContext in
            ...
            for feedGuid in feedGuids {
                ...
                for channelAsset in channel.assets ?? Set() {
                    guard let assetTypeRaw = channelAsset.type, let assetType = DPAGChannel.AssetType(rawValue: assetTypeRaw) else {
                        continue
                    }
    ...
    func updateFeedAssets(feedAssetDictionary: [String: Any]) -> [String: [DPAGChannel.AssetType]] {
        ...
        DPAGApplicationFacade.persistance.saveWithBlock { localContext in
            ...
            for (feedAssetIdent, feedAssetDataObj) in feedAssetDictionary {
                ...
                guard feedAssetIdents.count == 2 else {
                    continue
                }
    ...
    private func mapAssetResponseIdentToAssetParameters(assetResponseIdent: String) -> (channelId: String, assetType: DPAGChannel.AssetType)? {
        ...
        guard let assetType = DPAGChannel.AssetType(rawValue: components[1]) else {
            return nil
        }
    ...
    private func extractChannelAsset(channelGuid: String, assetType: DPAGChannel.AssetType, localContext: Any?) -> SIMSChannelAsset? {
        guard let localContext = localContext as? NSManagedObjectContext,
            let channel = SIMSChannel.findFirst(byGuid: channelGuid, in: localContext) else { return nil }
