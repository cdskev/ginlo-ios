...
#import "DPAGXmlReader.h"

NSString *const kDPAGXMLReaderTextNodeKey		= @"text";
NSString *const kDPAGXMLReaderAttributePrefix	= @"@";

@interface DPAGXmlReader ()
...
@implementation DPAGXmlReader

#pragma mark - Public methods

+ (NSDictionary *)dictionaryForXMLData:(NSData *)data error:(NSError **)error
{
    DPAGXmlReader *reader = [[DPAGXmlReader alloc] initWithError:error];
...
+ (NSDictionary *)dictionaryForXMLString:(NSString *)string error:(NSError **)error
{
    ...
    return [DPAGXmlReader dictionaryForXMLData:data error:error];
}
...
{
    NSData *data = [string dataUsingEncoding:NSUTF8StringEncoding];
    return [DPAGXmlReader dictionaryForXMLData:data options:options error:error];
}
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([self.textInProgress length] > 0)
    {
        ...
        [dictInProgress setObject:[trimmedString mutableCopy] forKey:kDPAGXMLReaderTextNodeKey];
