...
public class DPAGBackupFileInfo {
    ...
    func existStream(name: String) throws -> Bool {
        guard let zipFile = self.zipFile else {
            return false
        }
    ...
    func loadAndDecryptStream(name: String) throws -> [Any]? {
        ...
        try autoreleasepool {
            guard let data = try self.decryptedData() else {
                return
            }
    ...
    func decryptedData() throws -> Data? {
        ...
        try autoreleasepool {
            guard let aesKey = self.aesKey, let data = try self.encryptedData() else {
                return
            }
    ...
    static func loadZipData(zipStream: OZZipReadStream) throws -> Data {
        ...
        repeat {
            guard let buffer = NSMutableData(length: 31 * 1_024) else {
                break
            }
    ...
    func getFileNames() throws -> [String] {
        ...
        guard let zipFileInfos = try zipFile.listFileInZipInfosWithError() as? [OZFileInZipInfo] else {
            return []
        }
    ...
    func loadAttachment(fileName: String, attachment dataUrl: URL) throws {
        ...
        try autoreleasepool {
            ...
            if try zipFile.locateFile(inZip: fileNameAttachments, error: ()) == OZLocateFileResultFound {
                ...
                repeat {
                    guard let buffer = NSMutableData(length: 63 * 1_024) else {
                        break
                    }
...
class DPAGBackupWorker: DPAGBackupWorkerProtocol, DPAGClassPerforming {
    ...
    func createKey(pwd: String) throws {
        ...
        DPAGApplicationFacade.persistance.saveWithBlock { localContextSave in
            ...
            guard let account = SIMSAccount.mr_findFirst(in: localContextSave) else {
                return
            }
    ...
    func loadKeyConfig() -> Bool {
        ...
        DPAGApplicationFacade.persistance.loadWithBlock { localContext in
            ...
            guard let account = SIMSAccount.mr_findFirst(in: localContext) else {
                return
            }

            guard let salt = account.getAttribute("pbdkfSalt") as? String else {
                return
            }

            guard let rounds = account.getAttribute("pbdkfRounds") as? String else {
                return
            }

            guard let backupKey = account.getAttribute("backupKey") as? String else {
                return
            }

            guard let backupPasstoken = account.getAttribute("backupPasstoken") as? String else {
                return
            }
    ...
    func accountBackup(zipFile: OZZipFile?, writeOutput backupString: inout String?, backupMode mode: DPAGBackupMode) throws {
        ...
        try DPAGApplicationFacade.persistance.loadWithError { localContext in
            ...
            if mode == .fullBackup {
                ...
                // backupPasstoken einbauen
                innerData["backupPasstoken"] = self.backupPassToken
            } else {
                // MINIBackup Anpassen für WebClient
                ...
                if let companyPublicKey = companyInfo["publicKey"] {
                    ...
                    if mode != .miniBackupTempDevice {
                        // Anpassung WebClient : Raw - Keys übertragen
                        ...
                        // MDM Config Für das Temp Device sichern ...
    ...
    func contactBackup(zipFile: OZZipFile?, writeOutput backupString: inout String?, backupMode mode: DPAGBackupMode, deletedCompanyContactsOnly: Bool) throws {
        ...
        try DPAGApplicationFacade.persistance.loadWithError { localContext in
            ...
            for contact in allContacts {
                ...
                guard contactsWritten[contactGuid] == nil else {
                    continue
                }
                ...
                if deletedCompanyContactsOnly {
                    guard contact.isDeleted, contact.entryTypeServer == .company else {
                        return
                    }
                }
    ...
    func channelBackup(zipFile: OZZipFile) throws {
        try DPAGApplicationFacade.persistance.loadWithError { localContext in
            ...
            for channel in allChannels {
                guard let channelGuid = channel.guid, channel.subscribed?.boolValue ?? false else {
                    continue
                }
    ...
    func serviceBackup(zipFile: OZZipFile) throws {
        try DPAGApplicationFacade.persistance.loadWithError { localContext in

            let pred = NSComparisonPredicate(leftExpression: NSExpression(forKeyPath: \SIMSChannel.feedType), rightExpression: NSExpression(forConstantValue: DPAGChannelType.service.rawValue))
            ...
            for channel in allChannels {
                ...
                if let messages = channel.stream?.messages {
                    for message in messages {
                        if let messageChannel = message as? SIMSChannelMessage {
                            if let backupData = try self.convertChannelMessageToBackupJson(message: messageChannel, zip: zipFile) {
                                rc.append(backupData)
                            }
                        }
                    }
                }
                ...
                let fileName = channelGuid.replacingOccurrences(of: ":", with: "_").appending(".json")
    ...
    func singleChatBackup(zipFile: OZZipFile?, writeOutput backupString: inout String?, backupMode mode: DPAGBackupMode, includeDeletedCompanyContactsConversation: Bool) throws {
        ...
        try DPAGApplicationFacade.persistance.loadWithError { localContext in
            ...
            for stream in allStreams {
                guard stream.lastMessageDate != nil else {
                    continue
                }

                guard let contact = stream.contactIndexEntry, let contactGuid = contact.guid else {
                    continue
                }

                if includeDeletedCompanyContactsConversation {
                    guard contact.entryTypeServer == .company || (stream.wasDeleted?.boolValue ?? false) == false else {
                        continue
                    }
                } else {
                    guard (stream.wasDeleted?.boolValue ?? false) == false else {
                        continue
                    }
                }
    ...
    func singleChatBackup(messagestream: SIMSStream, zip zipFile: OZZipFile?, writeOutput backupString: inout String?, backupMode mode: DPAGBackupMode, in localContext: NSManagedObjectContext) throws {
        guard let contactGuid = messagestream.contactIndexEntry?.guid else {
            return
        }
    ...
    func writeFile(fileName: String, inZipFile zipFile: OZZipFile, andData jsonData: Data) throws {
        try autoreleasepool {
            guard self.bAppWillResignActive == false, let aesKey = self.aesKey else {
            ...
            // immer nur stückchenweise schreiben
    ...
    func saveAttachment(fileUrl: URL, withFileName fileNameIn: String, into zipFile: OZZipFile) throws {
        ...
        try autoreleasepool {
            guard self.bAppWillResignActive == false, let source = FileHandle(forReadingAtPath: fileUrl.path) else {
                throw DPAGErrorBackup.errCrypto
            }
    ...
    func groupChatBackup(messagestream: SIMSGroupStream, zip zipFile: OZZipFile?, writeOutput backupString: inout String?, backupMode mode: DPAGBackupMode, in localContext: NSManagedObjectContext) throws {
        guard let group = messagestream.group, let groupGuid = group.guid else {
            return
        }
    ...
    func convertGroupMessageToBackupJson(message: SIMSGroupMessage, zip zipFile: OZZipFile?, writeOutput _: inout String?, backupMode mode: DPAGBackupMode) throws -> [AnyHashable: Any]? {
        guard let guid = message.guid else {
            return nil
        }
        ...
        // Interne Messages (Gruppenbeitritt usw. mit sichern)
    ...
    func createBackupPasstoken() throws {
        guard let accountGuid = DPAGApplicationFacade.cache.account?.guid else {
            return
        }
    ...
    func makeAutomaticBackup() {
        guard let lastBackup = DPAGApplicationFacade.preferences.backupLastDate else {
            return
        }
        guard DPAGApplicationFacade.preferences.isBackupDisabled == false else {
            return
        }
    ...
    func makeBackup(hudWithLabels: DPAGProgressHUDWithProgressDelegate?) -> Bool {
        guard let account = DPAGApplicationFacade.cache.account, let contact = DPAGApplicationFacade.cache.contact(for: account.guid), let accountID = contact.accountID else {
            return true
        }
        ...
        do {
            ...
            guard let directoryPath = DPAGFunctionsGlobal.pathForCustomTMPDirectory() else {
                throw DPAGErrorBackup.errNoTmpDir
            }
            ...
            if DPAGApplicationFacade.preferences.isChannelsAllowed {
                ...
                try self.serviceBackup(zipFile: zipFile)
            }
            ...
            if let ubiquityContainer = fileManager.url(forUbiquityContainerIdentifier: nil) {
                ...
                repeat {
                    ...
                    for fileName in files {
                        ...
                        guard fileName.hasPrefix("info") == false else {
                            continue
                        }
                        _ = try fileInfo.loadAndDecryptStream(name: fileName)
                    }
                    // Folgende Dateien müssen da sein:
    ...
    func deleteOldBackups(backupPath: URL, skipFile backupFile: URL) {
        do {
            ...
            // Die letzte Synchronisierte Datei suchen
                ...
                if self.isSavedICloud(fileUrl: fileUrl) {
                    if hasValidBackup {
                        if fileUrl == backupFile {
                            ...
                            // Wenn löschen nicht klappt, dann ist es nicht ganz so schlimm
                    ...
                    // Noch nicht synchronisierte Dateien können wir löschrn
                    if fileUrl == backupFile {
                        DPAGLog("Skipping File %@", fileUrl.path)
                    } else {
                        DPAGLog("Deleting File %@", fileUrl.path)
    ...
    func setProgressState(step: Int, maxSteps max: Int, andText text: String, hudWithLabels: DPAGProgressHUDWithProgressDelegate?) {
        guard let hudWithLabels = hudWithLabels else {
            return
        }
    ...
    func recoverBackup(backupFileInfo: DPAGBackupFileInfo, hudWithLabels: DPAGProgressHUDWithProgressDelegate) throws {
        ...
        // SIMSme Kontakte vom Server laden
        ...
        // TimedMessages vom Server holen
        ...
        if DPAGApplicationFacade.preferences.isChannelsAllowed {
            ...
            // Channels wiederherstellem
            try self.recoverChannels(backupFileInfo: backupFileInfo)

            if DPAGApplicationFacade.preferences.isServicesAllowed {
                try self.loadServices()

                // Services wiederherstellem
                try self.recoverServices(backupFileInfo: backupFileInfo)
            }
        }

        // Server Bild und status laden
        ...
        // APN - Identifier zurücksetzen
        DPAGApplicationFacade.preferences.apnIdentifier = ""

        // InApp Pushes iweder enabled
        DPAGApplicationFacade.preferences.isInAppNotificationEnabled = true
    }

    func checkPassword(backupFileInfo: DPAGBackupFileInfo, withPassword password: String) throws {
        guard let filePath = backupFileInfo.filePath, let pbdkfSalt = backupFileInfo.pbdkfSalt, let salt = Data(base64Encoded: pbdkfSalt, options: .ignoreUnknownCharacters) else {
            throw DPAGErrorBackup.errNoConfig
        }
        ...
        guard let accountJson = try JSONSerialization.jsonObject(with: decryptedData, options: .allowFragments) as? [AnyHashable: Any], let accountInnerJson = accountJson["AccountBackup"] as? [AnyHashable: Any] else {
            throw DPAGErrorBackup.errFileInvalid
        }
        ...
        //            backupFileInfo.PhoneNumber = phone;
        //            backupFileInfo.EMailAddress = eMailAddress;
    ...
    func deleteBackups(accountID: String) throws {
        ...
        for fileInfo in array {
            if let filePath = fileInfo.filePath {
                // Alte Backups, die Sowieso nicht mehr wiederhergestellt werden können aus der Cloud löschen
    ...
    func listBackups(accountIDs: [String], orPhone phone: String?, queryResults: [Any], checkContent: Bool) throws -> [DPAGBackupFileInfo] {
        ...
        for result in queryResults where result is NSMetadataItem {
            if let aResult = result as? NSMetadataItem {
                ...
                guard namePart.hasPrefix("backup-"), namePart.hasSuffix(".simsbck") else {
                    continue
                }
    ...
    private func loadBackupFileInfo(fileURL: URL, name: String, fileSize: NSNumber?) -> DPAGBackupFileInfo? {
        do {
            ...
            if let jsonDict = (jsonData as? [AnyHashable: Any])?["BackupInfo"] as? [AnyHashable: Any] {
                guard let version = jsonDict["version"] as? String, version == "1" || version == "2" else {
                    return nil
                }
    ...
    func recoverAccount(backupFileInfo: DPAGBackupFileInfo) throws {
        ...
        guard let accountInfo = arr?.first as? [AnyHashable: Any], let innerAccountInfo = accountInfo["AccountBackup"] as? [AnyHashable: Any] else {
            return
        }
        ...
        try DPAGApplicationFacade.persistance.saveWithError { localContext in
            ...
            guard let account = SIMSAccount.mr_findFirst(in: localContext), let contact = SIMSContactIndexEntry.mr_createEntity(in: localContext), let device = SIMSDevice.mr_findFirst(in: localContext), let simsKey = SIMSKey.mr_findFirst(in: localContext) else {
                throw DPAGErrorBackup.errNoAccount
            }

            guard let accountGuid = innerAccountInfo["guid"] as? String else {
            ...
            guard let privateKey = innerAccountInfo["privateKey"] as? String, let contactPublicKey = contact.publicKey, let backupPasstoken = innerAccountInfo["backupPasstoken"] as? String else {
                throw DPAGErrorBackup.errAccountData
            }
            ...
            // [device setAttributeWithKey:SIMS_OBJECT_ATTR_NAME andValue:[DPAGHelper getDeviceName]];
            ...
            guard let recoverData = String(data: jsonData, encoding: .utf8) else {
                throw DPAGErrorBackup.errJsonEncoding
            }
    ...
    func loadOwnAdressInformation(account: SIMSAccount, in localContext: NSManagedObjectContext) {
        guard let accountGuid = account.guid else {
            return
        }
        ...
        guard let serverAdresses = serverAdressesResponse.responseArray else {
            return
        }

        guard let emailDomain = DPAGApplicationFacade.cache.contact(for: accountGuid)?.eMailDomain, let aesKey = try? account.aesKey(emailDomain: emailDomain) else {
            return
        }

        for serverAdress in serverAdresses {
            guard let kvPre = serverAdress as? [AnyHashable: Any], let kv = kvPre["AdressInformation"] as? [AnyHashable: Any] else {
                continue
            }

            guard kv["guid"] as? String != nil, let keyIv = kv["key-iv"] as? String, let data = kv["data"] as? String, kv["publicKey"] as? String != nil else {
                continue
            }

            guard let jsonData = try? CryptoHelperDecrypter.decrypt(encryptedData: data, withAesKey: aesKey, andIv: keyIv) else {
                continue
            }
    ...
    func recoverContacts(backupFileInfo: DPAGBackupFileInfo?, fromMinibackup backupData: [[AnyHashable: Any]]?, backupMode mode: DPAGBackupMode) throws {
        ...
        if mode == .fullBackup, let backupFileInfo = backupFileInfo {
            guard try backupFileInfo.existStream(name: "contacts.json") else {
                return
            }
        ...
        guard let contactsToRecover = contacts else {
            return
        }
        ...
        DPAGApplicationFacade.persistance.saveWithBlock { localContext in
            ...
            for contactBackup in contactsToRecover {
                guard let innerContact = (contactBackup["ContactBackup"] ?? contactBackup["DeletedCompanyContactBackup"]) as? [AnyHashable: Any] else {
                    continue
                }

                guard let guid = innerContact["guid"] as? String ?? innerContact["accountGuid"] as? String else {
                    continue
                }

                guard let contact = SIMSContactIndexEntry.findFirst(byGuid: guid, in: localContext) ?? SIMSContactIndexEntry.mr_createEntity(in: localContext) else {
                    continue
                }
    ...
    func recoverPrivateMessage(singleMessage: [AnyHashable: Any], backupFileInfo: DPAGBackupFileInfo?, accountInfo account: SIMSAccount, contactInfo contact: SIMSContactIndexEntry, orderId nextOrderId: UInt64, backupMode mode: DPAGBackupMode, in localContext: NSManagedObjectContext) throws {
        guard let guid = singleMessage["guid"] as? String else {
            return
        }

        guard let privateMessage = (SIMSMessage.findFirst(byGuid: guid, in: localContext) ?? SIMSPrivateMessage.mr_createEntity(in: localContext)) as? SIMSPrivateMessage else {
            return
        }
    ...
    func recoverTimedPrivateMessage(singleMessage: [AnyHashable: Any], backupFileInfo: DPAGBackupFileInfo?, accountInfo account: SIMSAccount, contactInfo contact: SIMSContactIndexEntry, orderId _: UInt64, backupMode mode: DPAGBackupMode, in localContext: NSManagedObjectContext) throws {
        guard let guid = singleMessage["guid"] as? String, let streamGuid = contact.stream?.guid else {
            return
        }

        guard let privateMessage = SIMSMessageToSendPrivate.findFirst(byGuid: guid, in: localContext) ?? SIMSMessageToSendPrivate.mr_createEntity(in: localContext) else {
            return
        }
    ...
    func recoverTimedPrivateMessageFromServer(timedMessage: [AnyHashable: Any], in localContext: NSManagedObjectContext) throws {
        guard let guid = timedMessage["guid"] as? String, let messageDataStr = timedMessage["data"] as? String else {
            return
        }

        guard let privateMessage = SIMSMessageToSendPrivate.findFirst(byGuid: guid, in: localContext) ?? SIMSMessageToSendPrivate.mr_createEntity(in: localContext) else {
            throw DPAGErrorBackup.errDatabase
        }

        guard let messageData = messageDataStr.data(using: .utf8) else {
            throw DPAGErrorBackup.errEncoding
        }

        guard let jsonData = try JSONSerialization.jsonObject(with: messageData, options: .allowFragments) as? [AnyHashable: Any] else {
            throw DPAGErrorBackup.errJsonEncoding
        }

        guard let singleMessage = jsonData["PrivateMessage"] as? [AnyHashable: Any] else {
            throw DPAGErrorBackup.errFileInvalid
        }

        privateMessage.guid = guid

        guard let fromDict = singleMessage["from"] as? [AnyHashable: Any], let fromAccountGuid = fromDict.keys.first as? String, let fromKey = (fromDict[fromAccountGuid] as? [AnyHashable: Any])?["key"] as? String else {
            throw DPAGErrorBackup.errFileInvalid
        }

        guard let toDict = (singleMessage["to"] as? [[AnyHashable: Any]])?.first, let toAccountGuid = toDict.keys.first as? String, let toKey = (toDict[toAccountGuid] as? [AnyHashable: Any])?["key"] as? String else {
            throw DPAGErrorBackup.errFileInvalid
        }
        ...
        // only the first recipient will be saved
        privateMessage.toAccountGuid = toAccountGuid
        privateMessage.toKey = toKey

        // NSString* signatureValid = [singleMessage objectForKey:@"signatureValid"];
    ...
    func recoverSingleChatStreamInternal(backupFileInfo: DPAGBackupFileInfo?, stream: [[AnyHashable: Any]], backupMode mode: DPAGBackupMode) throws {
        try DPAGApplicationFacade.persistance.saveWithError { localContext in
            ...
            for jsonDict in stream {
                // 1. Element ist das MetaObject
                if contactStream == nil, let singleChatBackupInfo = jsonDict["SingleChatBackup"] as? [AnyHashable: Any] {
                    guard var guid = singleChatBackupInfo["guid"] as? String else {
                        throw DPAGErrorBackup.errDataInvalid
                    }

                    if guid.hasPrefix(.streamGroup) {
                        // WorkAround WebClient
                    ...
                    guard let contact = SIMSContactIndexEntry.findFirst(byGuid: guid, in: localContext) ?? DPAGApplicationFacade.contactFactory.newModel(accountGuid: guid, publicKey: nil, in: localContext) else {
                        throw DPAGErrorBackup.errDatabase
                    }
                ...
                guard let contact = contactStream else {
                    break
                }
    ...
    func recoverGroupChatStreamInternal(backupFileInfo: DPAGBackupFileInfo?, existingServerGroups: [String: [AnyHashable: Any]], stream: [[AnyHashable: Any]], backupMode mode: DPAGBackupMode) throws -> [String: [AnyHashable: Any]] {
        ...
        try DPAGApplicationFacade.persistance.saveWithError { localContext in
            ...
            guard let account = SIMSAccount.mr_findFirst(in: localContext), let accountGuid = account.guid else {
                throw DPAGErrorBackup.errDatabase
            }
            ...
            for jsonDict in stream {
                ...
                if groupRecovered == nil {
                    guard let groupChatBackupInfo = jsonDict["ChatRoomBackup"] as? [AnyHashable: Any] else {
                        continue
                    }

                    guard let guid = groupChatBackupInfo["guid"] as? String else {
                        continue
                    }

                    guard let aesKey = groupChatBackupInfo["aes_key"] as? String, let iv = groupChatBackupInfo["iv"] as? String else {
                        continue
                    }

                    guard let stream = (SIMSMessageStream.findFirst(byGuid: guid, in: localContext) ?? SIMSGroupStream.mr_createEntity(in: localContext)) as? SIMSGroupStream else {
                        continue
                    }

                    guard let group = SIMSGroup.findFirst(byGuid: guid, in: localContext) ?? SIMSGroup.mr_createEntity(in: localContext) else {
                        continue
                    }
                    ...
                    // Vertrauensstatus aktualisieren
                ...
                guard let group = groupRecovered else {
                    break
                }
    ...
    func recoverGroupChatStream(backupFileInfo: DPAGBackupFileInfo) throws {
        ...
        if mutableDict.isEmpty == false {
            try DPAGApplicationFacade.persistance.loadWithError { localContext in
                ...
                // Gruppen verlassen, in denen ich nicht mehr Mitglied bin ..
    ...
    func recoverGroupMessage(groupMessageDict: [AnyHashable: Any], backupFileInfo: DPAGBackupFileInfo?, accountInfo account: SIMSAccount, contactInfo group: SIMSGroup, orderId nextOrderId: UInt64, backupMode mode: DPAGBackupMode, in localContext: NSManagedObjectContext) throws {
        guard let groupGuid = group.guid, let guid = groupMessageDict["guid"] as? String else {
            throw DPAGErrorBackup.errDataInvalid
        }

        guard let groupMessage = (SIMSMessage.findFirst(byGuid: guid, in: localContext) ?? SIMSGroupMessage.mr_createEntity(in: localContext)) as? SIMSGroupMessage else {
            throw DPAGErrorBackup.errDatabase
        }
        ...
        if let receiver = groupMessageDict["receiver"] as? [[AnyHashable: Any]], receiver.isEmpty == false {
            for receiverDictObj in receiver {
                guard let dictReceiver = receiverDictObj["Receiver"] as? [AnyHashable: Any] else {
                    continue
                }

                guard let receiverGuid = dictReceiver["guid"] as? String else {
                    continue
                }

                if groupMessage.receiver?.filter({ $0.contact?.guid == receiverGuid }).isEmpty ?? true {
                    guard let contact = SIMSContactIndexEntry.findFirst(byGuid: receiverGuid, in: localContext) else {
                        continue
                    }

                    guard let messageReceiver = SIMSMessageReceiver.mr_createEntity(in: localContext) else {
                        continue
                    }
    ...
    func recoverTimedGroupMessage(groupMessageDict: [AnyHashable: Any], backupFileInfo: DPAGBackupFileInfo?, accountInfo account: SIMSAccount, contactInfo group: SIMSGroup, orderId _: UInt64, backupMode mode: DPAGBackupMode, in localContext: NSManagedObjectContext) throws {
        guard let guid = groupMessageDict["guid"] as? String, let streamGuid = group.stream?.guid, let groupGuid = group.guid else {
            return
        }

        guard let groupMessage = SIMSMessageToSendGroup.findFirst(byGuid: guid, in: localContext) ?? SIMSMessageToSendGroup.mr_createEntity(in: localContext) else {
            return
        }
    ...
    func recoverTimedGroupMessageFromServer(timedMessage groupMessageDict: [AnyHashable: Any], in localContext: NSManagedObjectContext) throws {
        guard let guid = groupMessageDict["guid"] as? String else {
            return
        }

        guard let to = groupMessageDict["to"] as? String, let messageDataStr = groupMessageDict["data"] as? String, let messageData = messageDataStr.data(using: .utf8) else {
            return
        }

        guard let group = SIMSGroup.findFirst(byGuid: to, in: localContext), let streamGuid = group.stream?.guid else {
            return
        }
        ...
        guard let jsonData = try JSONSerialization.jsonObject(with: messageData, options: .allowFragments) as? [AnyHashable: Any], let groupMessageDict = jsonData["GroupMessage"] as? [AnyHashable: Any] else {
            return
        }

        // NSDictionary* fromDict = [groupMessageDict objectForKey:@"from"];

        // NSString* fromAccountGuid = [[fromDict allKeys] objectAtIndex:0];

        // NSString* fromKey = [[fromDict objectForKey:fromAccountGuid] objectForKey:@"key"];
        ...
        // groupMessage.from = fromKey;

        // only the first recipient will be saved
        groupMessage.toGroupGuid = to
        // NSString* signatureValid = [groupMessageDict objectForKey:@"signatureValid"];
    ...
    func recoverKeys(fromDict dict: [AnyHashable: Any], forGroupMessage message: SIMSManagedObjectMessage) {
        ...
        guard let fromDict = dict["from"] as? [AnyHashable: Any], let fromAccountGuid = fromDict.keys.first as? String, let to = dict["to"] as? String else {
            return
        }
    ...
    func recoverAttachment(fromDict dict: [AnyHashable: Any], forMessage message: SIMSManagedObjectMessage, accountInfo account: SIMSAccount, backupFileInfo: DPAGBackupFileInfo) throws {
        guard let attachment = dict["attachment"] as? [String], attachment.isEmpty == false else {
            return
        }

        // Attachment wieder herstellen
        guard let attachmentGuid = attachment.first else {
            return
        }
        guard let dataUrl = AttachmentHelper.attachmentFilePath(guid: attachmentGuid) else {
            return
        }
    ...
    func recoverSignature(fromDict dict: [AnyHashable: Any], forMessage message: SIMSManagedObjectMessage, accountInfo account: SIMSAccount) {
        guard let signature = dict["signature"] as? [AnyHashable: Any] else {
            return
        }

        guard let hashes = signature["hashes"] as? [AnyHashable: Any] else {
            return
        }

        guard let attachmentSha = hashes["attachment/0"] as? String else {
            return
        }

        guard let deviceCrypto = CryptoHelper.sharedInstance else {
            return
        }

        guard let key = account.keyRelationship else {
            return
        }

        message.attachmentHash = try? deviceCrypto.encrypt(string: attachmentSha, with: key)

        guard let signature256 = dict["signature-sha256"] as? [AnyHashable: Any] else {
            return
        }

        guard let hashes256 = signature256["hashes"] as? [AnyHashable: Any] else {
            return
        }

        guard let attachmentSha256 = hashes256["attachment/0"] as? String else {
            return
        }
        ...
        try DPAGApplicationFacade.persistance.saveWithError { localContext in
            ...
            if serverMessageGuids.isEmpty == false {
                for i in stride(from: 0, to: serverMessageGuids.count, by: 10) {
                    ...
                    for message in messages {
                        // TimedMessage vom Server wiederherstellen
                        guard let innerDict = message["TimedMessage"] as? [AnyHashable: Any] else {
                            continue
                        }

                        guard let guid = innerDict["guid"] as? String else {
                            continue
                        }

                        guard (innerDict["data"] as? String) != nil else {
                            continue
                        }

                        guard (innerDict["sendDate"] as? String) != nil else {
                            continue
                        }
    ...
    func convertAttachment(for message: SIMSManagedObjectMessage, toDict dict: inout [AnyHashable: Any], zipFile: OZZipFile?) throws {
        if let attachment = message.attachment {
            //  Attachments normalisieren und exportieren
    ...
    func convertKeys(for messageObj: SIMSManagedObjectMessage, toDict dict: inout [AnyHashable: Any]) {
        if let message = messageObj as? SIMSPrivateMessage {
            ...
            // fromKey brauchen wir nicht zu sichern, da Obsolet
            let fromKeyDict = ["key": ""]
            let fromDict = [message.fromAccountGuid: fromKeyDict]
        ...
        let responseBlock: DPAGServiceResponseBlock = { responseObject, errorCode, errorMessage in
            ...
            if let errorMessage = errorMessage {
                ...
                DPAGApplicationFacade.persistance.saveWithBlock { localContext in
                    ...
                    for channel in rc {
                        guard let channelShort = channel["Channel"] as? [AnyHashable: Any] else {
                            continue
                        }

                        guard let guid = channelShort["guid"] as? String else {
                            continue
                        }

                        guard let channel = SIMSChannel.findFirst(byGuid: guid, in: localContext) ?? SIMSChannel.mr_createEntity(in: localContext) else {
                            continue
                        }
    ...
    func loadServiceGuids() throws -> [String] {
        var channelGuids: [String] = []
        var errorCodeBlock: String?
        var errorMessageBlock: String?
        ...
        let responseBlock: DPAGServiceResponseBlock = { responseObject, errorCode, errorMessage in
            ...
            if let errorMessage = errorMessage {
                ...
                DPAGApplicationFacade.persistance.saveWithBlock { localContext in
                    ...
                    for channel in rc {
                        guard let channelShort = channel["Service"] as? [AnyHashable: Any] else {
                            continue
                        }
                        ...
                        channel.name_short = shortDesc
                        channel.checksum = checksum
                        channel.feedType = NSNumber(value: DPAGChannelType.service.rawValue)
                    }
                }
            }
        }
    ...
    func loadServices() throws {
        DPAGLog("Start Loading Services")

        let channelGuids = try self.loadServiceGuids()
        ...
        DPAGApplicationFacade.feedWorker.updateFeeds(feedGuids: channelGuids, feedType: .service, feedUpdatedBlock: { _, errorCode, errorMessage in
        ...
        DPAGLog("Finished Loading Services")
    }

    func recoverChannels(backupFileInfo: DPAGBackupFileInfo) throws {
        guard let channels = try backupFileInfo.loadAndDecryptStream(name: "channels.json") as? [[AnyHashable: Any]] else {
            return
        }
        ...
        try DPAGApplicationFacade.persistance.saveWithError { localContext in
            ...
            for channelBackup in channels {
                guard let innerChannel = channelBackup["ChannelBackup"] as? [AnyHashable: Any] else {
                    continue
                }

                guard let guid = innerChannel["guid"] as? String else {
                    continue
                }

                guard let channel = SIMSChannel.findFirst(byGuid: guid, in: localContext) else {
                    continue
                }

                guard let channelStream = (SIMSMessageStream.findFirst(byGuid: guid, in: localContext) ?? SIMSChannelStream.mr_createEntity(in: localContext)) as? SIMSChannelStream else {
                    continue
                }
                ...
                for opt in channelOptions {
                    guard let ident = opt.ident else {
                        continue
                    }
    ...
    func recoverServices(backupFileInfo: DPAGBackupFileInfo) throws {
        var channelGuids: [String] = []
        var channelsToSend: [[AnyHashable: Any]] = []

        let allFileNames = try backupFileInfo.getFileNames()

        for filename in allFileNames where filename.hasPrefix("22_") {
            guard let channels = try backupFileInfo.loadAndDecryptStream(name: filename) as? [[AnyHashable: Any]] else {
                continue
            }
            ...
            try DPAGApplicationFacade.persistance.saveWithError { localContext in
                ...
                var channelStreamFound: SIMSChannelStream?

                for channelBackup in channels {
                    if let innerChannel = channelBackup["ChannelBackup"] as? [AnyHashable: Any] {
                        guard let guid = innerChannel["guid"] as? String else {
                            continue
                        }
                        ...
                        channelStreamFound = channelStream
                        ...
                        channelsToSend.append(["Service": innerDict])
                    }

                    if let channelStream = channelStreamFound, let innerChannelMessage = channelBackup["ChannelMessage"] as? [AnyHashable: Any] {
                        try self.recoverChannelMessage(singleMessage: innerChannelMessage, backupFileInfo: backupFileInfo, accountInfo: account, channelStream: channelStream, orderId: orderId, in: localContext)
                        orderId += 1
                    }
                }
            }
        ...
        try self.setFollowedServices(channelInfos: channelsToSend)
        self.loadServiceAssets(serviceGuids: channelGuids)
    }
    ...
    func recoverChannelMessage(singleMessage: [AnyHashable: Any], backupFileInfo: DPAGBackupFileInfo, accountInfo account: SIMSAccount, channelStream: SIMSChannelStream, orderId nextOrderId: UInt64, in localContext: NSManagedObjectContext) throws {
        ...
        guard let channelMessage = (SIMSMessage.findFirst(byGuid: guid, in: localContext) ?? SIMSChannelMessage.mr_createEntity(in: localContext)) as? SIMSChannelMessage, let channelGuid = channelStream.channel?.guid else {
            return
        }
    ...
    func loadServiceAssets(serviceGuids: [String]) {
        let semaphore = DispatchSemaphore(value: 0)

        DPAGApplicationFacade.feedWorker.updateAssets(feedGuids: serviceGuids, feedType: .service) {
            // defer {
            semaphore.signal()
            // }
        }
    ...
    func recoverAccount(innerAccountInfo: [AnyHashable: Any], accountGuid: String, deviceGuid: String, publicKey pubKey: String, publicKeyFingerPrint publicKeySig: String) throws -> String {
        ...
        try DPAGApplicationFacade.persistance.saveWithError { localContext in
            ...
            guard let account = SIMSAccount.mr_createEntity(in: localContext), let device = SIMSDevice.mr_createEntity(in: localContext), let simsKey = SIMSKey.mr_createEntity(in: localContext), let contact = SIMSContactIndexEntry.mr_createEntity(in: localContext) else {
                throw DPAGErrorBackup.errDatabase
            }
            ...
            /*
             NSString* backupPasstoken = [NSString dynamicDowncast:[innerAccountInfo objectForKey:@"backupPasstoken"]];
             [account setAttributeWithKey:@"backupPasstoken" andValue:backupPasstoken];*/

            account.accountState = .recoverBackup

            guard let privateKey = innerAccountInfo["privateKey"] as? String else {
                throw DPAGErrorBackup.errAccountData
            }
            ...
            if let companyInfo = innerAccountInfo["companyInfo"] as? [AnyHashable: Any], let companyGuid = companyInfo["guid"] as? String {
                ...
                // TODO: publicKey oder companyPublicKey ?!
            ...
            // SharedInstance muss auf deviceCrypto gesetzt werden ...
    ...
    func recoverMiniBackup(miniBackup: String, accountGuid: String, deviceGuid: String, deviceName deviceNameEncoded: String, publicKey pubKey: String, publicKeyFingerPrint publicKeySig: String, transId: String) throws {
        try autoreleasepool {
            guard let miniBackupData = miniBackup.data(using: .utf8) else {
                throw DPAGErrorBackup.errDataInvalid
            }
            ...
            // TODO: Weitere Daten vom Server laden ....
            try self.recoverAccountInfo()
        }
    }
}
